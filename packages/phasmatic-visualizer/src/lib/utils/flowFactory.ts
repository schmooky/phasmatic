import { PHASE_REGISTRY, PhaseMetadata } from 'phasmatic';
import { 
  FlowData, 
  FlowNode, 
  FlowEdge, 
 ...
 */
export function generateFlowFromStateMachine(
  ...
  phaseRegistry.forEach((metadata, phaseName) => {
    ...
    if (metadata.possibleNextPhases.length > 1 && config.showConditions !== false) {
      ...
      const edgeToCondition: FlowEdge = {
        id: `edge-${edgeId++}`,
        source: phaseName,
  ...
  nodes.forEach(node => {
    if (node.id === 'start') {
      ...
      const sourceNode = nodes.find(n => n.id === (node.data as ConditionNodeData).metadata?.sourcePhase);
      if (sourceNode) {
        node.position = { 
